// Codeforces 117C done (too much effort)
package main
...
func findCycle(i int, lev int) int {
	//fmt.Println("findCycle, i,lev=",i,lev)
    curc[lev] = i
    u.setBit(i)
    tou.setBit(i)
    for j:=0; j<n; j++ {
        if aa[i].isSet(j) {            
            if u.isSet(j) {
                curc[lev+1] = j
                curc[lev+2] = -1
	            //fmt.Println("foundCycle, j,lev=",j,lev)
                return (lev+1)
            } 
            if tou.isSet(j) {
            	continue
...
func main() {
	...
	//fmt.Println("Cycles read data passed")
    // looking for cycles
    for i:=0; i<n; i++ {
        if !tou.isSet(i) {
        	//fmt.Println("look for cycles from", i)
        	if findCycle(i,0) >= 0 {
        		a,b,c := findThree()
	            fmt.Printf("%d %d %d\n", a+1,b+1,c+1)
	            return
