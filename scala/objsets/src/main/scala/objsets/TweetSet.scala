...
class Empty extends TweetSet {
  ...
  // -------------------------------------------------------------------------
  def ascendingByRetweet = new EmptyTrending
  override def toString = "_empty_"
}

class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {

  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    val accuElem = if (p(elem)) accu.incl(elem) else accu
    left.filter0(p, right.filter0(p, accuElem))
  }
  ...
  def union(that: TweetSet): TweetSet = {
    if (that.isEmpty) this
      ...
      val h = that.head
      val t = that.tail
      // println("adding to " + this + " from h: " + h.text + " t: " + t)
      incl(h).union(t)
    }
  }

  def ascendingByRetweet = {
    val mn = findMin
    new NonEmptyTrending(mn, remove(mn).ascendingByRetweet)
  }
...
class NonEmptyTrending(elem: Tweet, next: Trending) extends Trending {
   ...
   */
  def +(tw: Tweet): Trending =
    new NonEmptyTrending(elem, tail + tw)
  def head: Tweet = elem
...
object GoogleVsApple {
  ...
  def mentionedl(l: List[String], t: Tweet): Boolean =
    if (l.isEmpty) false
    else if (t.text.contains(l.head)) true 
    else mentionedl(l.tail, t)

  def mentioned(l: List[String]): Tweet => Boolean = t => mentionedl(l, t)

  val googleTweets: TweetSet = TweetReader.allTweets.filter(mentioned(google))
  val appleTweets: TweetSet = TweetReader.allTweets.filter(mentioned(apple))
