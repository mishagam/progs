...
public class KorWordsConf {
	...
	public void confFromBundle(Bundle b) {
		...
		prefVersionCode = b.getInt("pref_version_code", 0);
	...
	public void confToBundle(Bundle b) {
		...
		b.putInt("pref_version_code", prefVersionCode);
	...
	public List<Integer> getDictResIds(Context c) {
		String []ss = dictResIds.split(",");
		List<Integer> li = new ArrayList<Integer>();
		for (String sid : ss) {
			int resId = c.getResources().getIdentifier(sid,  "raw", c.getPackageName());
			if (resId == 0) {
				throw new RuntimeException("Illegal dictionary name: " + sid);
			}
			li.add(resId);
		}
		return li;
	...
	public int getPrefVersionCode() {
		return prefVersionCode;
	}

	public void setPrefVersionCode(int prefVersionCode) {
