...
import mis.android.game.GameActivity;
import mis.android.game.GameSurfaceView;
import mis.android.game.WorldSelectActivity;
...
public class MisGame {
    ...
    public enum GameMode {
        NORMAL, MOVE_SCENE, ROTATE_SCENE, ADD_BRICKS;
    }
    ...
    int ww;
    int hh;
    public static MisContext myContext;
    ...
    // default system specific values 
    public static float DEFAULT_Z = 80f;
    public static float DEFAULT_SCENE_Y = 50f;
    ...
    public MisGame(MisContext context, GameSurfaceView view) {
        myContext = context;
        this.view = view;
    ...
    public void playLog() {
        try {
            BufferedReader br = new BufferedReader(new FileReader(MOUSE_LOG_FILENAME));
            String s;
            long lastMillis = 0;
    ...
    public void onMove(float x, float y) {
        ...
        if (gameMode == GameMode.MOVE_SCENE) {
            moveScene(x, y);
            return;
        } else if (gameMode == GameMode.ROTATE_SCENE) {
            rotateScene(x,y);
            return;
    ...
    public void onSurfaceCreated(MisGL11 gl, MisEGLConfig config) {
        // initialize game
        // init world
        w = new World(this);
        w.readWorld(myContext, worldName);
        this.view = view;
        // init graphics
         initTextures(gl);
         initTopPanel();
    ...
    private void topPanelLogMenu() {
        ...
        topPanel.clear();
        topPanel.addGsButton("nearer", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
                onDistanceChange(-10);                    
            }
        });
        topPanel.addGsButton("further", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
                onDistanceChange(+10);                    
            }
        });
        topPanel.addGsButton("turn", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
                onTurnStorage();                    
            }
        });
        ...
        for (String slog : logSizes) {
            if (slog.length() == 4) {
                final String s = slog;
                topPanel.addGsButton(slog, new OnClickListener() {
                    public void onClick(GameSurfaceView v) {
        ...
        topPanel.addGsButton("back", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
    ...
    private void topPanelMainMenu() {
        ...
        topPanel.addGsButton("nearer_further", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
                onDistanceChange(-10);                    
            }
        });
        ...
        if (w.containsFigure("brick")) {
            topPanel.addGsButton("onebrickbtn1", new OnClickListener() {
                public void onClick(GameSurfaceView v) {
            ...
            topPanel.addGsButton("onebrickbtn", new OnClickListener() {
                public void onClick(GameSurfaceView v) {
                    Log.i("MisGame", "Brick Selected");
        ...
        if (w.containsFigure("log")) {
            topPanel.addGsButton("log1", new OnClickListener() {
                public void onClick(GameSurfaceView v) {
        ...
        rotateButton = topPanel.addButton2("rotate_btn", "rotate_btn_sel", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
                Log.i("MisGame", "Rotate Scene Selected");   
                if (gameMode == GameMode.ROTATE_SCENE) {
                    gameMode = GameMode.NORMAL;
                    rotateButton.setCurrentImage(0);
                } else {
                    gameMode = GameMode.ROTATE_SCENE;
                    rotateButton.setCurrentImage(1);
                }
                requestRender();
            }
        });
        ...
        topPanel.addGsButton("window_btn_1", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
        ...
        topPanel.addGsButton("window_btn_2", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
                Log.i("MisGame", "window_btn_1 Selected");                  
        ...
        topPanel.addGsButton("door_btn", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
                Log.i("MisGame", "window_btn_1 Selected");                  
        ...
        topPanel.addGsButton("roof_btn", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
        ...
        topPanel.addGsButton("back", new OnClickListener() {
            public void onClick(GameSurfaceView v) {
                ...
                // switching to game select menu
                Intent intent = new Intent(myContext.getContext(), WorldSelectActivity.class);
    ...
    private void initGameSelectMenu() {
        ...
        selectGame.addGsButton(gameName0, new OnClickListener() {
            public void onClick(GameSurfaceView v) {
        ...
        selectGame.addGsButton(gameName1, new OnClickListener() {
            public void onClick(GameSurfaceView v) {
    ...
    public void onRotateButton(int dax) {
        mAngleZ += dax;  
        projChanged = true;
        requestRender();
    }
    ...
    private void startRotateScene(float x, float y) {
        MVector3f a = new MVector3f();
        convertPosZ(x, y, SCENE_Z, a);
        lastSceneX = a.x;
        lastSceneY = a.y;
    ...
    private void rotateScene(float x, float y) {
        if (Math.abs(lastSceneX) < 0.1 && Math.abs(lastSceneY)<0.1 ) {
            return;
        }
        float angleCoef = 0.33f;
        MVector3f a = new MVector3f();
        convertPosZ(x, y, SCENE_Z, a);
        mAngleZ += angleCoef*(a.x - lastSceneX);
        mAngleX -= angleCoef*(a.y - lastSceneY);
        lastSceneX = a.x;
        lastSceneY = a.y;
        requestRender();
    }
    ...
    private void endRotateScene(float x, float y) {
        if (Math.abs(lastSceneX) < 0.1 && Math.abs(lastSceneY)<0.1 ) {
            return;
        }
        moveScene(x,y);
