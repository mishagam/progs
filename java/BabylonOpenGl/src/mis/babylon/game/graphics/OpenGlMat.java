...
public class OpenGlMat {
    ...
    static void matLoadArray(MMatrix4f mat, float []ma) {
    ...
    static void matStoreArray(MMatrix4f mat, float []ma) {
        FloatBuffer buf = FloatBuffer.allocate(16);
        mat.store(buf);
        buf.get(ma);
    }
    ...
    static int glhInvertMatrixf2(float[] m, float[] out) {
        MMatrix4f mm = new MMatrix4f();
        MMatrix4f mout = new MMatrix4f();
        matLoadArray(mm, m);
        if (MMatrix4f.invert(mm, mout) == null) {
            return 0;
        } else {
            matStoreArray(mout, out);
            return 1;
        }
    }
}
