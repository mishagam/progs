...
(function ($) {
  ...
  /**
   * Add padding of 0s in front of the number;
   *
   * @param num
   *  Number.
   * @param places
   ...
   * @returns {string}
   */
  function schedulerZeroPad(num, places) {
    var zero = places - num.toString().length + 1;
    return new Array(+(zero > 0 && zero)).join("0") + num;
  }

  /**
   * Returns the number of seconds until publication for a given region and date.
   */
  function schedulerCountdown(region, publishDate) {
    // In lieu of dynamic variables, doing matching here.
    var timezone;
    switch (region) {
      case "EU":
        timezone = Drupal.settings.darwin_scheduler.darwin_scheduling_EU_timezone;
        break;

      case "US":
        timezone = Drupal.settings.darwin_scheduler.darwin_scheduling_US_timezone;
        break;

      case "AP":
        timezone = Drupal.settings.darwin_scheduler.darwin_scheduling_AP_timezone;
        break;
    ...
    // Get the hour at which we publish e.g. 6am.
    var publishHour = Drupal.settings.darwin_scheduler.publish_hour;
    // Set the date and time this issue should be published locally.
    var publishTime = moment(publishDate + ' ' + publishHour + ':00:00').format('YYYY-M-D, HH:mm:ss');
    // Get local time for the region we're looking at.
    var localNow = moment().tz(timezone).format('YYYY-MM-DD HH:mm:ss');
    // Find difference between LocalNow and publish time.
    return moment(publishTime).diff(localNow);
  }

  /**
   * Returns an hour, minute, second countdown for a milliseconds value.
   */
  function schedulerMillisecToCountdown(milliseconds) {
    var duration = moment.duration(milliseconds, 'milliseconds');
    var hours = Math.floor(duration.asHours());
    var mins = Math.floor(duration.asMinutes()) - hours * 60;
    var secs = Math.floor(duration.asSeconds()) - (hours * 60 * 60) - (mins * 60);
    return hours + "h " + schedulerZeroPad(mins, 2) + "m " + schedulerZeroPad(secs, 2) + "s ";
  }

  Drupal.behaviors.darwin_scheduler = {
    attach: function (context, settings) {
      // For each View row.
      $('div.views-row', context).each(function (index, item) {
        // Generate the publish date and time based on date and time zone.
        var $issueCountdown = $(this).find('.issue-countdown');
        var region = $issueCountdown.attr("region");
        var publishDate = $issueCountdown.attr("date");

        // Get the number of milliseconds until publication.
        var diff = schedulerCountdown(region, publishDate);
        // Get workflow state.
        var workflowState = $issueCountdown.attr("state");

        // If time is up, say we're published.
        if (diff < 0 && workflowState == "Approved") {
          $issueCountdown.text("Published");
        }
        else if (diff < 0 && workflowState == "Unapproved") {
          $issueCountdown.text('Unpublished');
          $issueCountdown.addClass("Unpublished");
        }
        else {
          // Store the number of milliseconds as an attribute.
          $issueCountdown.attr("count", diff);
          // Set a countdown if we have a number.
          (function (element) {
            setInterval(function () {
              var count = element.attr("count");
              element.text(schedulerMillisecToCountdown(count - 1000));
              element.attr("count", count - 1000);
            }, 1000);
          })($issueCountdown);
        }
      });
