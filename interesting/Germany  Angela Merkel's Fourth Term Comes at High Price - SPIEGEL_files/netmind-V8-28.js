...
function config_getPixelUrl () {
    var protocol = this.m_strPixelProtocol;
    if (protocol == 'about') //in einem iframe srcdoc ausgelöst
    {
        protocol = "http";
    }
	return protocol+"://"+this.m_strPixelServer + this.m_strPixelPath;
}
...
function spIvwReload(this_ivwcode) {
	if( typeof iom != "undefined" && typeof iam_data != "undefined" ) {
		if(this_ivwcode) { 
			iam_data.cp = this_ivwcode; 
		}
		iom.c(iam_data, 1);
	}
}
