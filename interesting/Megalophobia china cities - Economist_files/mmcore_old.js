...
//transport
    ...
    _rt:1000,
//private
    jsver:'5.15.2',_vars:{fv:{},act:{},uat:{},ids:{}},_act_id:0,_vars_alias:{act:'uv'},_renderers:{},_extensions:{},_r_mbs:{},
    ...
    _ReadParams:function(str,f,r)/*f-field, r-record separators*/{
        ...
        for(i=0;i<rs.length;i++){kv=rs[i].split(f);if(kv.length!=2)continue;
            kv[0]=_t._TL(kv[0]);if(kv[0].indexOf(p)==0&&/^[.\w]+$/.test(kv[0])){var n = kv[0].substr(p.length),v=_t._T(kv[1]);v=(n=='uat'?unescape(v):v);_t.SetParam(n, v);}}},
