...
var protocol=doc.location.protocol.toLowerCase()=="https:"?"https:":"http:",
baseUrl=(function(){
if(win.begun_urls&&window.begun_urls.base_scripts_url){
return win.begun_urls.base_scripts_url;
}else{
...
Begun.loaderCallbacks.push(function(){
if(!Begun||!Begun.Scripts||!Begun.Scripts.addStrictFunction){
return;
}
Begun.Scripts.addStrictFunction(initLoadingFeed);
appendScript(baseUrl+"acp/"+"begun_utils"+"."+initScripts["begun_utils"]+".js");
});

function initLoadingFeed(){
if(!Begun.Scripts||!Begun.Utils||Begun.Autocontext){
return;
}
...
if(typeof be.sent[errorMessage]!=='undefined'||be.excludedErrors[errorMessage]){
return;
}

var defaultErrorLogger=baseUrl+'log_errors?',
address=window.begun_error_url||defaultErrorLogger;
...
'&pad_id='+Begun.Autocontext.Pads.getPad().pad_id+'&location='+encodeURI(document.URL)+
'&rev='+Begun.VERSION
);
};
...
this.savedFeeds={};
this.savedFeedsCallbacks={};
this.loadingFeedsFinished=false;
...
included=Begun.Utils.includeScript(link,'append');
}
...
if(warningText.hasOwnProperty(key)){
...
"begun_tpl_block_mediatext_price_horizontal":"e83242a411",
...
_this.tplLoaded.notFinished[tplFileName]=Begun.Utils.includeScript(_this.Strings.urls.base_scripts_url+"acp/"+
tplFileName+"."+ext[tplFileName]+".js",'append');
}
},
loadAll:function(){
...
base_scripts_url:baseUrl,
daemon:baseUrl+'context.jsp?',
rtb_daemon:protocol+'//rtb.begun.ru',
video:protocol+'//video.begun.ru/vcp.swf',
thumbs:protocol+'//thumbs.begun.ru/',
blank:baseUrl+'img/blank.gif',
mediatext_price_background:baseUrl+'img/background_',
log_banners_counter:baseUrl+'blockcounter?log_visibility=1&',
...
currentBlock=_this.blocksDataList[_this.blocksDataIndex],
padId=_this.requestParams.pad_id=currentBlock.pad_id;
...
var blockEl=_this.Blocks.getDomObj(currentBlock.block_id);
...
_this.requestParams.block_id+=comma+currentBlock.block_id;
}
...
_this.requestParams.stopwords=currentBlock.stopWords||'';
_this.requestParams.begun_self_keywords=currentBlock.keywords||'';
_this.requestParams.begun_self_categories=currentBlock.categories||'';
_this.requestParams.format_w=currentBlock.format_w||'';
_this.requestParams.format_h=currentBlock.format_h||'';
_this.requestParams.do_cpm=currentBlock.do_cpm?1:0;
...
var small_images=[],i,l,
cells=Begun.Utils.getElementsByClassName(rich_blocks_div,'*','begun_adv_rich'),
ln=0;
...
doc.head.appendChild(linksFrame);
}
};
...
if(blocksLength==0){
Begun.Error.send("No blocks on the page",doc.URL,-1);
return;
}

for(var i=0;i<blocksLength;i++){
blockEl=blockElements[i];
auctionCallbackKey=blockEl.getAttribute('begun-callback-key');
auction=blockEl.getAttribute('begun-auction');
padId=blockEl.getAttribute('begun-auto-pad');
blockId=blockEl.getAttribute('begun-block-id');
keywords=blockEl.getAttribute('begun-self-keywords');
stopWords=blockEl.getAttribute('begun-stopwords');
categories=blockEl.getAttribute('begun-self-categories');
conditionId=blockEl.getAttribute('begun-condition-id');
formatW=blockEl.getAttribute('begun-format-w');
formatH=blockEl.getAttribute('begun-format-h');
doCpm=blockEl.getAttribute('begun-cpm');
clickWrap=blockEl.getAttribute('begun-click-wrap');
...
'auction_key':auctionCallbackKey,
'auction':auction
};
...
if(data.message=='savedFeed'&&data.feed){
var feed=data.feed;
window.begunAds=feed.feed;
blockData.block_id=feed.block_id;
blockData.pad_id=feed.pad_id;
blocksDataList.push(blockData);
_this.setBlockId(blockEl,blockData.block_id);
_this.Pads.init(blocksDataList);
_this.blocksDataList=blocksDataList;
_this.blocksDataIndex=0;
_this.loadFeedDone();
}else if(data.message=='getSavedFeed'){
_this.savedFeedsCallbacks[data.feedKey]=(function(source,key){
return function(){
source.postMessage(JSON.stringify({
'message':'savedFeed',
'feed':_this.savedFeeds[key]
}),'*');
}
})(event.source,data.feedKey);
if(_this.savedFeeds[data.feedKey]){
_this.savedFeedsCallbacks[data.feedKey]();
}
}
}
}

Begun.Utils.addEvent(window,"message",receiveMessage);

if(auctionCallbackKey){
if(window.top!=window){
window.top.postMessage(JSON.stringify({
'message':'getSavedFeed',
'feedKey':auctionCallbackKey
}),'*');
}
break;
}else{
blocksDataList.push(blockData);
_this.setBlockId(blockEl,blockId);
}
}
if(!auctionCallbackKey){
_this.Pads.init(blocksDataList);
...
this.loadNextBlock=function(){
if(_this.blocksDataList[_this.blocksDataIndex+1]){
_this.loadingFeedsFinished=false;
_this.blocksDataIndex++;
_this.loadFeed();
}else{
_this.loadingFeedsFinished=true;
}
};

this.addBlock=function(params,callback){
...
auction=true;
padId=params['begun-auto-pad'];
blockId=params['begun-block-id'];
formatW=params['begun-format-w'];
formatH=params['begun-format-h'];
blockData={
'pad_id':padId,
'block_id':blockId,
'format_w':formatW,
...
if(!_this.blocksDataList){
_this.blocksDataList=[];
_this.blocksDataIndex=0;
function receiveMessage(event){
try{
var data=JSON.parse(event.data);
...
Begun.Utils.addEvent(window,"message",receiveMessage);
_this.blocksDataList.push(blockData);
_this.Pads.addPad(blockData);
_this.loadFeed();
}else{
_this.blocksDataList.push(blockData);
_this.Pads.addPad(blockData);
if(_this.loadingFeedsFinished){
_this.loadNextBlock();
}
}
};
...
if(_this.blocksDataList[_this.blocksDataIndex].click_wrap){
vars.url=_this.blocksDataList[_this.blocksDataIndex].click_wrap+vars.url;
}
...
this.Banners.infoValues.setAll(block_id,graph_banner);
...
docWriteRegexp=/^document\.write\((.*)\);$/g,
matches,codePlace;
...
matches=docWriteRegexp.exec(code);
if(matches&&matches[1]){
codePlace=_this.Blocks.getDomObj(block_id);
codePlace.innerHTML=matches[1]||'';
Begun.Utils.reattachScripts(codePlace.childNodes);
...
if(!feed){
Begun.Error.send('Attempted to load an undefined feed.',document.URL,-1);
return null;
}

this.registerGraphBannerDomains(feed);

if(_this.blocksDataList[_this.blocksDataIndex].auction){
function countLevel(feed){
var level=0,
bannersCount=0;
...
var impression_id=feed.params.impression_id,
countedLevel=countLevel(feed),
blockData=_this.blocksDataList[_this.blocksDataIndex];
this.savedFeeds[impression_id]={
'pad_id':blockData.pad_id,
'block_id':blockData.block_id,
'feed':feed
};
if(this.savedFeedsCallbacks[impression_id]){
this.savedFeedsCallbacks[impression_id]();
}
this.setFeed(feed);
this.Callbacks.dispatch('auction','start',this,[countedLevel,impression_id,blockData.block_id]);
if(blockData.callback&&typeof blockData.callback=='function'){
blockData.callback(countedLevel,impression_id,blockData.block_id);
}
_this.loadNextBlock();
return;
}
...
if(options&&options.dimensions&&options.dimensions.type){
...
addFakeBlocks();
_this.Blocks.push(_this.Blocks.getBlockById(_this.blocksDataList[_this.blocksDataIndex].block_id,feed.blocks));
...
_this.insertNonTextBlock(_this.Blocks.getBlockById(_this.blocksDataList[_this.blocksDataIndex].block_id,feed.blocks));
_this.draw();
};
}(this));

this.setBlockId=function(block,block_id){
block.id=_this.Strings.css.block_prefix+block_id;
};

this.printDefaultStyle=function(){
Begun.Utils.includeStyle(_this.Tpls.getCSS('default'),'append');
};
...
return _this.Pads.getPad(pad_id);
};
...
var vars={},
block_hover_html='',
block_opts=block.options.visual||{};
...
banner&&_this.Banners.infoValues.setAll(banner.block_id,banner);
...
this.Callbacks.dispatch('block','predraw',this,[block]);
...
this.Callbacks.dispatch('banner','predraw',this,[banner]);

if(banner.block_id&&!banner.disabled&&banner.block_id==block.id){
addHTML(block,banner,banners_html,block_banner_count);
block_banner_count++;
}
i++;
}
...
if(block_banner_count<banners_count&&block_banner_count!=0&&isValidSquareBlock(block)){
...
_this.blocksDataList.push({
'block_id':BLOCK_ID_FULLSCREEN,
'pad_id':pad_id
});
...
var showUrl=ac.Banners.infoValues.get(blockId,bannerId,'show_url');
if(showUrl){
_this.send('',showUrl);
}
};
...
block.hidden=true;
data.push({pad_id:padId},logData);
}
}
}
}
...
var pads=[];
this.init=function(blocksArr){
var i,blocksLength=blocksArr.length;
for(i=0;i<blocksLength;i++){
this.addPad(blocksArr[i]);
}
};
this.getPad=function(pad_id){
if(!pads.length){
...
if(!pad_id){
pad_id=ac.blocksDataList[ac.blocksDataIndex].pad_id;
}
...
this.addPad=function(block){
if(!this.getPad(block.pad_id)){
pads.push({
pad_id:block.pad_id,
feed:null,
blocks:[]
...
var block,
i=0,
blocks=ac.getPad(pad_id).blocks;
...
html['search_banner_html']='<iframe src="{{source}}" width="{{width}}" height="{{height}}" scrolling="no" frameborder="0" marginheight="0" marginwidth="0" hspace="0" vspace="0" style="{{styles}}" {{sandbox}}></iframe>';
...
if(/webkit/i.test(u)||(/mozilla/i.test(u)&&!/(compati)/.test(u))||(/opera/i.test(u))){
setTimeout(function(){
...
appendScript(baseUrl+"acp/"+"begun_scripts"+"."+initScripts["begun_scripts"]+".js");
})();