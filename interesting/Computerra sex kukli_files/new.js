...
//    $( "<div class='item-pic-hover'></div>" ).prependTo( obj ).hide().fadeIn(100);
/*	$( obj ).find('.item-pic').prepend( "<div class='item-pic-hover'></div>" );
    $( obj ).find( 'div.item-pic-hover' ).fadeIn(100);
    $( obj ).find( 'div.item-soc' ).fadeIn(100);
    $( obj ).find( 'div.edit-link' ).fadeIn(100);
   	$( obj ).find( 'a.item-section' ).addClass('item-section-hover');*/
}
function hideItem(obj) {
    $( obj ).find( 'span.item-author' ).hide();
    $( obj ).find( 'span.item-date-inner' ).show();
//    $( obj ).find( "div:first" ).fadeOut(100);
/*    $( obj ).find( 'div.item-pic-hover' ).remove();
    $( obj ).find( 'div.item-soc' ).fadeOut(100);
    $( obj ).find( 'div.edit-link' ).fadeOut(100);
    $( obj ).find( 'a.item-section' ).removeClass('item-section-hover');
*/
}
...
//    $( this ).parent().find('.item-popup').css('height', h + 'px');
...
/*	var inp = $( this ).parent().parent().find("input.item-soc-link-input");
	alert(inp.val());
	inp.selectionStart = 0;
	inp.selectionEnd = 999;*/
    return false;
});
...
/*
$(document).ready(function(){
	$(".item-soc-link-button").on('mouseover', function(){
		$(this).off('mouseover');
		$('.item-soc-link-button').zclip({
			path:'http://www.computerra.ru/files/other/ZeroClipboard.swf',
			copy:function(){return $(this).attr('title');},
	...
	$(".item-soc-link-link").on('mouseover', function(){
		$(this).off('mouseover');
		$('.item-soc-link-link').zclip({
			path:'http://www.computerra.ru/files/other/ZeroClipboard.swf',
			copy:function(){return $(this).attr('title');},
			afterCopy:function(){
				$(this).text("Ссылка скопирована");
...
//	        if (hash == 0) history.pushState(null, null, "");
//	        else history.pushState(null, null, 'page/' + hash);
        	curhash = hash;
        }
    });
});
...
$(document).ready(function(){
//if ($('.adbantop').height() > 50) {
/*  	$('.top-submenu').css({'position' : 'absolute' , 'top' : 54});*/
  	$('.top-submenu-cl').css({'position' : 'absolute' , 'top' : 54});
//  	$('#change-ban').css({'position' : 'absolute' , 'top' : 295});
//}
});
...
/*  	$('.top').css({'position' : 'absolute' , 'top' : 200});
  	...
  	$('.top-submenu-cl').css({'position' : 'absolute' , 'top' : 54});
*/
//  	$('#change-ban').css({'position' : 'absolute' , 'top' : 185});
/*  	$('#change-ban').css({'position' : 'absolute' , 'top' : 255});*/
} else {
  if($(window).scrollTop() < 75) {
//  	$('#change-ban').css({'position' : 'absolute' , 'top' : 55});
  }
}
...
/*  if($(window).scrollTop() < 101) {
  	$('.top').css({'position' : 'absolute' , 'top' : 90});
  	$('.top-submenu').css({'position' : 'absolute' , 'top' : 54});
  	$('.top-submenu-cl').css({'position' : 'absolute' , 'top' : 54});
  	$('#change-ban').css({'position' : 'absolute' , 'top' : 180});
  }
...
if (ww <= 10) {
    ...
    if ( $("#adriver_banner_1832222208").length){
